...
public:
	...
	CombinedFeatures _combFeat;  //features will be used for future
	CAction _lastAction;
	PNode _score;
...
public:
	inline void computeNextScore(Graph *cg, const vector<CAction>& acs){
		if (_bStart){
			_nextscores.forward(cg, acs, _combFeat, NULL);
		}
		else{
			_nextscores.forward(cg, acs, _combFeat, _score);
		}
	}

	inline void collectFeat(ModelParams* model_params){
		_prevState->_combFeat.collectFeatures(model_params);
	}
